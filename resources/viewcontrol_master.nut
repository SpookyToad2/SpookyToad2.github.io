...
function DestroyCamera()
{
	if (hCamera != null && hCamera.IsValid())
	{
		hCamera.AcceptInput("Disable", "", null, null); // Ensure disabled
		hCamera.Kill();
	}
	hCamera = null;
}
...
function DisableCamera(player)
{
	...
	if (hCamera != null && hCamera.IsValid())
	{
		hCamera.AcceptInput("Disable", "", player, null);
	}
...
function DisableAndDestroyPerPlayer()
{
	foreach (idx, camera in hPlayerCameras)
	{
		...
		{
			camera.AcceptInput("Disable", "", null, null);
			camera.Kill();
		}
	}
...
function DisablePerPlayerCamera(player, camera)
{
	...
	if (camera != null && camera.IsValid())
	{
		camera.AcceptInput("Disable", "", player, null);
	}
